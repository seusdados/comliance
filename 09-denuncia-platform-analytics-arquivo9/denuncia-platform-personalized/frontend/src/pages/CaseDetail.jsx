...
function CaseDetail({ user }) {
  ...
  // Estado para armazenar dados descriptografados do caso
  const [decryptedCase, setDecryptedCase] = useState(null);
  ...
  const fetchCase = async () => {
    try {
      ...
      if (response.ok) {
        const c = data.case;
        setCaseData(c);
        // Se houver dados cifrados, tenta descriptografar usando a chave local
        if (c && c.encryptedData) {
          const key = localStorage.getItem(`case-key-${c.id}`);
          if (key) {
            const decrypted = decryptObject(c.encryptedData, key);
            if (decrypted) {
              setDecryptedCase(decrypted);
            } else {
              setDecryptedCase(null);
            }
          } else {
            setDecryptedCase(null);
          }
  ...
  return (
    <div>
      ...
      <div className="p-4 max-w-3xl mx-auto">
        ...
        {caseData && (
          <>
            {/* Título e descrição: usa dados descriptografados se disponíveis */}
            <h2 className="text-lg font-semibold mb-2">
              {decryptedCase && decryptedCase.title ? decryptedCase.title : caseData.title || 'Caso'}
            </h2>
            <p className="mb-4">
              {decryptedCase && decryptedCase.description
                ? decryptedCase.description
                : caseData.description || 'Sem descrição disponível'}
            </p>
            {/* Categorias */}
            {(() => {
              const cats = decryptedCase && decryptedCase.categories ? decryptedCase.categories : caseData.categories;
              return cats && cats.length > 0 ? (
                <p className="mb-2"><span className="font-semibold">Categorias:</span> {cats.join(', ')}</p>
              ) : null;
            })()}
            {/* Prioridade */}
            {(() => {
              const pri = decryptedCase && decryptedCase.priority ? decryptedCase.priority : caseData.priority;
              return pri ? (
                <p className="mb-2"><span className="font-semibold">Prioridade:</span> {pri.level} (score {pri.score})</p>
              ) : null;
            })()}
            <div className="mb-4">
