...
- **backend/** – aplicação Node que expõe uma interface HTTP para registro de casos, autenticação, chat e relatórios.  Inclui suporte a múltiplos clientes (multi‑tenant), controle de perfis, anonimato do denunciante e armazenamento em arquivos JSON para facilitar a instalação.  Agora traz módulos de IA (classificação de categoria e resumo simples), um cofre de identidades com criptografia AES‑256‑GCM e stubs de integração com Instagram, Facebook, WhatsApp e e‑mail.
  ...
  Esta versão inclui um **módulo de gestão de tarefas de investigação**, inspirado nas diretrizes das normas ISO 37008 e 37002.  Assim que um investigador é designado a um caso, o sistema gera automaticamente uma linha do tempo de tarefas (avaliação preliminar, definição de escopo, planejamento, coleta de evidências, entrevistas, elaboração de relatório, medidas corretivas etc.).  As tarefas podem ser acompanhadas, marcadas como concluídas e reprogramadas pela equipe responsável diretamente na página de detalhes do caso.  Também é possível adicionar tarefas personalizadas, definindo título, descrição e data de vencimento.
  ...
  **Personalização e usabilidade** – O backend oferece um endpoint `/api/settings` que permite consultar e atualizar as configurações de cada empresa (tenant). Essas configurações incluem:

  - **Categorias** e **fluxos de status** (etapas do ciclo de vida do caso);
  - **Tema** (cores primária e secundária) para personalizar a aparência da interface;
  - **Idiomas** disponíveis para a interface e formulários;
  - **Formulários** específicos para cada tipo de usuário (por exemplo, denunciantes, vítimas e consultores), definindo os campos exibidos, seus rótulos (mapeados em arquivos de tradução) e se são obrigatórios ou não.

  No frontend, a página de *Configurações* (acessível apenas a administradores e diretores) permite editar esses itens de forma intuitiva. A partir dessas definições:

  - A página de nova denúncia se adapta ao perfil selecionado, exibindo somente os campos relevantes de acordo com o formulário definido para aquele tipo de usuário. Campos de seleção podem reutilizar a lista de categorias definida pela empresa.
  - A interface inteira (menus, botões, mensagens) é traduzida dinamicamente para o idioma escolhido pelo usuário. Novos idiomas podem ser adicionados no arquivo `frontend/src/i18n.js` e habilitados nas configurações do tenant.
  - As cores da barra de navegação e outros elementos usam variáveis CSS (`--primary-color`, `--secondary-color`) que são preenchidas com os valores definidos nas configurações da empresa.

  Esta personalização garante que a experiência seja adequada a cada público e conforme as normas de acessibilidade, respeitando as diretrizes da ISO de que os canais de denúncia devem ser visíveis, acessíveis e disponíveis em linguagens apropriadas【938468719210140†L2155-L2171】【938468719210140†L1286-L1299】【989042326995849†L1852-L1867】.
