...
app.post('/api/cases/:id/tasks', authMiddleware, requireRole('admin', 'ceo', 'triage', 'investigator'), (req, res) => {
  ...
  saveData('cases.json', cases);
  return res.status(201).json({ mensagem: 'Tarefa adicionada', task });
});
...
app.patch('/api/cases/:id/tasks/:taskId', authMiddleware, requireRole('admin', 'ceo', 'triage', 'investigator'), (req, res) => {
  ...
  const { status, dueDate, description, assignedTo } = req.body;
  const caseIndex = cases.findIndex((c) => c.id === caseId && c.tenantId === req.tenantId);
  if (caseIndex < 0) {
    return res.status(404).json({ mensagem: 'Caso não encontrado' });
  }
  ...
  saveData('cases.json', cases);
  return res.json({ mensagem: 'Tarefa atualizada', task });
});
